class UMLClass {
  constructor(id, x, y, name = 'ClassName', attributes = [], methods = []) {
    ...
    this.width = 150;
    this.height = 100 + attributes.length * 15 + methods.length * 15; // Ajuste dinámico del tamaño basado en los atributos y métodos
    this.type = 'UMLClass';
  }

  // Método para dibujar bordes redondeados
  ...
  draw(context) {
    const borderRadius = 10; // Tamaño de las esquinas redondeadas
    ...
    // Color de fondo de la clase
    context.fillStyle = "#ffffff"; 
    context.strokeStyle = "#333"; // Borde de la clase
    context.lineWidth = 2;

    // Dibujar la caja de la clase con bordes redondeados
    this.drawRoundedRect(context, this.x, this.y, this.width, this.height, borderRadius);

    // Eliminar sombra para los textos y líneas internas
    context.shadowColor = "transparent";
    
    // Estilo de texto para el nombre de la clase
    context.font = "bold 14px Arial";
    context.fillStyle = "#333"; // Color del texto
    context.textAlign = "center";
    context.fillText(this.name, this.x + this.width / 2, this.y + 20); // Nombre de la clase centrado

    // Dibujar división debajo del nombre de la clase
    context.beginPath();
    context.moveTo(this.x, this.y + 25);
    context.lineTo(this.x + this.width, this.y + 25);
    context.stroke();

    // Estilo de texto para atributos y métodos
    context.font = "12px Arial";
    context.textAlign = "left";
    context.fillStyle = "#555"; // Color del texto de los atributos y métodos

    // Dibujar los atributos
    let attributesY = this.y + 40;
    for (let attribute of this.attributes) {
      context.fillText(attribute, this.x + 10, attributesY);
      attributesY += 15;
    }

    // Dibujar división debajo de los atributos
    context.beginPath();
    context.moveTo(this.x, attributesY);
    ...
    for (let method of this.methods) {
      context.fillText(method, this.x + 10, methodsY);
      methodsY += 15;
    }
  }
